...
from app.models.business_model import Business, BusinessCreate, BusinessSchema, BusinessCreateReturn
...
class BusinessController:
    ...
    def _get_current_user_business(self):
    ...
    async def get_business(self) -> BusinessSchema:
        ...
        business_id = self._get_current_user_business()
        business = await self._get_and_check_by_id(business_id)
        return BusinessSchema.model_validate(business)

    async def update_business(self, business_update: BusinessCreate) -> BusinessSchema:
        business_id = self._get_current_user_business()
    ...
    @superuser_only
    async def update_business_id(self, business_id: int, business_update: BusinessCreate) -> BusinessCreateReturn:
        business = await self._get_and_check_by_id(business_id)
        business = await self.business_service.update(business_id, business_update)
        return BusinessCreateReturn.model_validate(business)
