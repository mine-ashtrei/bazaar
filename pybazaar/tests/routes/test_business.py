...
class TestBusinessRoutes:
    ...
    #     response = client.post("/business", json=self._get_business_dict())
    #     assert response.status_code == 401, response.json()
    
    # def test_create_business(self, auth_client):
    #     # response = client.post("/business", json=self._get_business_dict(), headers={
    #     #     "Authorization": f"Bearer {jwt}"
    #     # })
    #     response = auth_client.post("/business", json=self._get_business_dict())
    #     print(response.json())
    #     assert response.status_code == 201, response.json()